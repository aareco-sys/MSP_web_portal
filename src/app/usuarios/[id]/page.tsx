...
import { useUserScorecard, useUserSkills } from "@/hooks/use-data";
...
type ViewTab = "performance" | "ficha";
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        ...
        <div className="view-toggle" data-noprint>
          <button type="button" className={cn(tab === "performance" && "is-active")} onClick={() => setTab("performance")}>
            {t.fichaTecnica.tabPerformance}
          </button>
          <button type="button" className={cn(tab === "ficha" && "is-active")} onClick={() => setTab("ficha")}>
            {t.fichaTecnica.tab}
          </button>
        </div>
      </div>
      ...
      {tab === "performance" ? (
        ...
        <FichaTecnicaTab userId={id} />
      )}
    </>
  );
}

function FichaTecnicaTab({ userId }: { userId: string }) {
  const t = useT();
  const { data, isLoading, error } = useUserSkills(userId);
  if (isLoading) return <Loading />;
  if (error) return <ErrorState message={(error as Error).message} />;
  if (!data?.ficha) return <Empty label={t.fichaTecnica.empty} />;
  return <FichaTecnica ficha={data.ficha} />;
}
